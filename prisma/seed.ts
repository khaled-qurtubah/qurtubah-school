...
async function main() {
  ...
  const programs: Array<{
    name: string; department: string; employee?: string; status: string;
    priority: string; progress: number; year: number; month: number;
  }> = []

  // === الإدارة المدرسية (11 برنامج) ===
  programs.push(
  ...
  // ═══════════════════════════════════════
  // 5. إنشاء مستخدم مدير افتراضي
  // ═══════════════════════════════════════
  await db.user.upsert({
    where: { email: 'admin@qurtubah.edu.sa' },
    update: {},
    create: {
      name: 'مدير مدارس قرطبة',
  ...
  // نسخ الشعار إلى مجلد public
  const fs = await import('fs')
  const path = await import('path')
  const sourceLogo = path.join(process.cwd(), 'upload', 'Qurtubah-Schools-New-Logo-1 (1).png')
  const destLogo = path.join(process.cwd(), 'public', 'logo-school.png')
  if (fs.existsSync(sourceLogo)) {
    fs.copyFileSync(sourceLogo, destLogo)
    console.log('✅ تم نسخ الشعار بنجاح')
  }
